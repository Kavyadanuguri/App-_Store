...
class AppStore extends Component {
  state = {tabValue: tabsList[0].tabId, inputValue: ''}
  ...
  render() {
    const {tabValue, inputValue} = this.state
    const filteredList = appsList.filter(each => each.category === tabValue)
    const finalList = filteredList.filter(each =>
      each.appName.toLowerCase().includes(inputValue.toLowerCase()),
    )
        ...
        <ul className="container2">
          {tabsList.map(each => (
            <TabItem
              ...
              tabDetails={each}
              isActive={tabValue === tabsList.tabId}
            />
          ))}
        </ul>
